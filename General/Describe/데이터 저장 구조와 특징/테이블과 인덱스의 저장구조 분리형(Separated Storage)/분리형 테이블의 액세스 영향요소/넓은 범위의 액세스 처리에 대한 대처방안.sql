 ...
 *  
 *    # 특히, 넓은 범위의 데이터를 조회할 때 랜덤 I/O가 증가하여 성능 저하가 발생할 수 있음.
 * 
 *  ㅇ 넓은 범위 액세스 시 성능 저하 원인
 *     1. 데이터가 연속적으로 저장되지 않음 -> 많은 블록을 랜덤하게 읽어야 함( 랜덤 I/O 증가 )
 *     2. 분리형 테이블은 클러스터형 인덱스가 없어 -> 범위 검색이 비효울적
 *     3. 다량의 데이터가 필요한 경우, Full Table Scan 발생 가능성 높음
 *
 *     # 따라서, 적절한 인덱스 구성 및 데이터 저장 방식 최적화가 필요합니다!
 * 
 *  [ 넓은 범위 액세스 성능 최적화 방법 ]
 ...
 *   v 주기적으로 테이블을 정렬하여 범위 검색 성능을 향상!
 * 
 * 
 ...
 *     ㅇ 넓은 범위 데이터를 조회할 때, 쿼리를 병혈로 실행하여 속도 개선 가능
 *
